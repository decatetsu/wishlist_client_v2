...
const validationSchema = object({
  name: string().label('Name').required().min(4).max(32),
  description: string().label('Description').min(0).max(64),
  link: string().label('Link').min(0).max(128),
  price: number().label('Price').required().min(0).max(Number.MAX_VALUE),
});
...
<template>
  <DefaultModal header="Create wish" :closeModal="closeModal">
