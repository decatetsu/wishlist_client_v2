...
const defaultName = 'CollectionX';

const name = ref<string>(defaultName);
const publicVisibility = ref<boolean>(false);

const validationSchema = object().shape({
  name: string().label('Name').notRequired().when('name', {
    is: (value) => value !== defaultName,
    then: (rule) => rule.min(3).max(16),
  }),
}, [
  ['name', 'name']
]);
...
<template>
  <DefaultModal header="Edit collection" :closeModal="closeModal">
